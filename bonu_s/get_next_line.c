...
/*   Updated: 2023/10/02 18:40:13 by momihamm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*therestofline(char *str)
{
	...
	therest = (char *) my_calloc (len + 1, sizeof (char));
	j = 0;
	while (str[indx] != '\n' && str[indx])
		indx++;
	if (str[indx] == '\n')
...
char	*reading(int fd, char *buff_read)
{
	...
	readbuff = (char *) my_calloc (BUFFER_SIZE + 1, sizeof (char));
	offset = 1;
	while (isthereanewline(readbuff) == 1 && offset > 0)
	{
		...
		readbuff[offset] = '\0';
		buff_read = my_join (buff_read, readbuff);
		if (buff_read[0] == '\0')
		{
			free(buff_read);
			free (readbuff);
...
char	*get_next_line(int fd)
{
	...
	if (!the_buff)
		free(the_buff);
	the_buff = my_sawebstr(the_line, ft_lenofstr(the_line, '\n') + 1, len);
	the_line = my_sawebstr(the_line, 0, ft_lenofstr(the_line, '\n') + 1);
	free (ptr_ofad);
	return (the_line);
}
